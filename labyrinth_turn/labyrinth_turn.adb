pragma Ada_2022;
package body Labyrinth_Turn is
   ...
   begin
      ...
      for I in 1 .. Count loop
         declare F, T, Diff : Natural; begin
            Next_Rand (State, F, 4); Next_Rand (State, T, 4);
            Trials (I).Facing := F; Trials (I).Target := T;
            Diff := (if F >= T then F - T else T - F);
            if Diff = 3 then Diff := 1; end if;
            Trials (I).Turns := Diff; end;
      end loop;
   end Build_Session;
   ...
   begin
      ...
      for I in 1 .. Count loop
         R.Score := R.Score + Trial_Score (Trials (I).Turns, Answers (I));
      end loop;
      return R;
   end Score_Session;
end Labyrinth_Turn;
