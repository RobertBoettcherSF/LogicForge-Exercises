pragma Ada_2022;
package body Eyewitness_Count is
   ...
   begin
      ...
      for I in 1 .. Count loop
         declare M, D, Tmp : Natural; begin Next_Rand (State, M, 16); M := M - 1;
            Trials (I).Mask := M; Tmp := M; D := 0;
            for K in 1 .. 4 loop D := D + (Tmp mod 2); Tmp := Tmp / 2; end loop;
            Trials (I).Details := D; end;
      end loop;
   end Build_Session;
   ...
   begin
      ...
      for I in 1 .. Count loop
         R.Score := R.Score + Trial_Score (Trials (I).Details, Answers (I));
      end loop;
      return R;
   end Score_Session;
end Eyewitness_Count;
