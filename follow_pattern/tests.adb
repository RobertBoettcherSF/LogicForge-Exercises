...
begin
   ...
   Put_Line ("TEST 1");
   Check ("1.1 hit", Trial_Score ('C', 'C') = 1);
   Check ("1.2 miss", Trial_Score ('C', 'D') = 0);
   Check ("1.3 cfg", Config_Ok ((8, 4, 1)));
   Put_Line ("TEST 2 — Config");
   Check ("2.1", Config_Ok ((8, 4, 5)));
   Check ("2.2", not Config_Ok ((Max_Trials + 1, 4, 1)));
   ...
   begin
      Build_Session ((8, 4, 5), T, C);
      for I in 1 .. C loop A (I) := T (I).Next_Letter; end loop;
   ...
   begin
      Build_Session ((8, 4, 5), A, CA);
      Build_Session ((8, 4, 5), B, CB);
      Check ("6.1", CA = CB);
      Check ("6.2", CA = CB);
      Check ("6.3", True);
   end;
   ...
   Check ("9.3", Instruction_Key (1 .. 14) = "follow_pattern");
   ...
   begin
      Build_Session ((8, 4, 5), T, C);
      for I in 1 .. C loop
         if I mod 2 = 0 then A (I) := T (I).Next_Letter;
         else A (I) := 'Z'; end if;
      end loop;
      R := Score_Session ((8, 4, 5), T, C, A);
