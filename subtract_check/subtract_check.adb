pragma Ada_2022;
package body Subtract_Check is
   ...
   begin
      ...
      for I in 1 .. Count loop
         declare
            A, B : Natural;
         begin
            Next_Rand (State, A, 50);
            A := A + 10;
            Next_Rand (State, B, A);
            Trials (I).Minuend := A;
            Trials (I).Subtrahend := B - 1;
            Trials (I).Difference := A - (B - 1);
         end;
   ...
   begin
      ...
      for I in 1 .. Count loop
         R.Score := R.Score + Trial_Score (Trials (I).Difference, Answers (I));
      end loop;
      return R;
   end Score_Session;
end Subtract_Check;
