...
with Letter_Index; use Letter_Index;
procedure Tests is
...
begin
   ...
   begin
      Build_Session ((8, 5), T, C);
      for I in 1 .. C loop A (I) := T (I).Index; end loop;
   ...
   begin
      Build_Session ((8, 5), T, C);
      for I in 1 .. C loop A (I) := T (I).Index + 97; end loop;
      R := Score_Session ((8, 5), T, C, A);
      Check ("5.1", R.Score = 0);
   ...
   Check ("9.3", Instruction_Key (1 .. 12) = "letter_index");
   ...
   begin
      Build_Session ((8, 5), T, C);
      for I in 1 .. C loop
         if I mod 2 = 0 then A (I) := T (I).Index; else A (I) := 0; end if;
      end loop;
      R := Score_Session ((8, 5), T, C, A);
