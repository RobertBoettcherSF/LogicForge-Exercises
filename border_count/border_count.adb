...
package body Border_Count is
   ...
   begin
      ...
      for I in 1 .. Count loop
         declare X, Y : Natural; begin Next_Rand (State, X, 12); Next_Rand (State, Y, 12);
            Trials (I).A := X; Trials (I).B := Y;
            Trials (I).Shared := (if X < Y then X else Y); end;
      end loop;
   end Build_Session;
   ...
   begin
      ...
      for I in 1 .. Count loop
         R.Score := R.Score + Trial_Score (Trials (I).Shared, Answers (I));
      end loop;
      return R;
   end Score_Session;
end Border_Count;
