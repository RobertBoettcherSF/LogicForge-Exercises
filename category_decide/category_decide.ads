...
package Category_Decide is
   Max_Trials : constant Positive := 24;

   type Category is (Animal, Plant, Object, Place);
   ...
   type Trial is record
      Label    : String (1 .. 10) := [others => ' '];
      Len      : Positive := 1;
      Expected : Category := Animal;
   end record;

   type Trial_List is array (1 .. Max_Trials) of Trial;
   type Answer_List is array (1 .. Max_Trials) of Category;
   ...
   Instruction_Key : constant String := "category_decide.instruction";
   Prompt_Key      : constant String := "category_decide.prompt_cat";

   function Trial_Score (Expected, Given : Category) return Natural
