...
package body Route_Steps is
   ...
   begin
      ...
      for I in 1 .. Count loop
         declare
            A, B, C, D : Natural;
         begin
            Next_Rand (State, A, 20);
            Next_Rand (State, B, 20);
            Next_Rand (State, C, 20);
            Next_Rand (State, D, 20);
            Trials (I).X1 := A; Trials (I).Y1 := B;
            Trials (I).X2 := C; Trials (I).Y2 := D;
            Trials (I).Steps := (if A >= C then A - C else C - A)
              + (if B >= D then B - D else D - B);
         end;
      end loop;
   end Build_Session;
   ...
   begin
      ...
      for I in 1 .. Count loop
         R.Score := R.Score + Trial_Score (Trials (I).Steps, Answers (I));
      end loop;
      return R;
   end Score_Session;
end Route_Steps;
