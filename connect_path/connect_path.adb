...
package body Connect_Path is
   ...
   begin
      ...
      for I in 1 .. Count loop
         declare A, B : Natural; begin Next_Rand (State, A, 15); Next_Rand (State, B, 15);
            Trials (I).Start_N := A; Trials (I).End_N := B;
            Trials (I).Nodes := (if A >= B then A - B else B - A) + 1; end;
      end loop;
   end Build_Session;
   ...
   begin
      ...
      for I in 1 .. Count loop
         R.Score := R.Score + Trial_Score (Trials (I).Nodes, Answers (I));
      end loop;
      return R;
   end Score_Session;
end Connect_Path;
