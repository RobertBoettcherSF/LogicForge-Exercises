...
with Max_Of_Three; use Max_Of_Three;
procedure Tests is
...
begin
   ...
   begin
      Build_Session ((8, 5), T, C);
      for I in 1 .. C loop A (I) := T (I).Max_Value; end loop;
   ...
   begin
      Build_Session ((8, 5), T, C);
      for I in 1 .. C loop A (I) := T (I).Max_Value + 97; end loop;
      R := Score_Session ((8, 5), T, C, A);
      Check ("5.1", R.Score = 0);
   ...
   begin
      ...
      Check ("6.1", CA = CB);
      Check ("6.2", A (1).Max_Value = B (1).Max_Value);
      Check ("6.3", True);
   end;
   ...
   Check ("9.3", Instruction_Key (1 .. 12) = "max_of_three");
   ...
   begin
      Build_Session ((8, 5), T, C);
      for I in 1 .. C loop
         if I mod 2 = 0 then A (I) := T (I).Max_Value; else A (I) := 0; end if;
      end loop;
      R := Score_Session ((8, 5), T, C, A);
