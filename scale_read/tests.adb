...
with Scale_Read; use Scale_Read;
procedure Tests is
...
begin
   ...
   begin
      Build_Session ((8, 5), T, C);
      for I in 1 .. C loop A (I) := T (I).Choice; end loop;
   ...
   begin
      Build_Session ((8, 5), T, C);
      for I in 1 .. C loop A (I) := T (I).Choice + 97; end loop;
      R := Score_Session ((8, 5), T, C, A);
      Check ("5.1", R.Score = 0);
   ...
   begin
      ...
      Check ("6.1", CA = CB);
      Check ("6.2", A (1).Choice = B (1).Choice);
      Check ("6.3", True);
   end;
   ...
   Check ("9.3", Instruction_Key (1 .. 10) = "scale_read");
   ...
   begin
      Build_Session ((8, 5), T, C);
      for I in 1 .. C loop
         if I mod 2 = 0 then A (I) := T (I).Choice; else A (I) := 0; end if;
      end loop;
      R := Score_Session ((8, 5), T, C, A);
