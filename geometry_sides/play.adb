...
procedure Play is
   ...
   Answers : Answer_List := [others => 3];
   Line : String (1 .. 80); Last : Natural; Result : Session_Result;
begin
   Put_Line ("[" & Instruction_Key & "]");
   Put_Line ("How many sides does the shape have?");
   Build_Session (Cfg, Trials, Count);
   for I in 1 .. Count loop
      New_Line;
      Put_Line ("Trial" & I'Image & " Shape: " & Trials (I).Kind'Image);
      Put ("[" & Prompt_Key & "]: ");
      Get_Line (Line, Last);
      begin Answers (I) := Positive'Value (Line (1 .. Last));
      exception when others => Answers (I) := 1; end;
   end loop;
