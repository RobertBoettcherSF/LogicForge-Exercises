...
package body Line_Measure is
   ...
   function Count_Filled (Cells : Row; Len : Positive) return Natural is
      N : Natural := 0;
   begin
      for I in 1 .. Len loop
         if Cells (I) = Filled then N := N + 1; end if;
      end loop;
      return N;
   end Count_Filled;
   ...
   begin
      ...
      for I in 1 .. Count loop
         ...
         Trials (I).Cells := [others => Empty];
         Next_Rand (State, Start, Cfg.Len);
         Next_Rand (State, Span, Cfg.Len - Start + 1);
         for J in Start .. Start + Span - 1 loop
            Trials (I).Cells (J) := Filled;
         end loop;
         Trials (I).Length := Count_Filled (Trials (I).Cells, Cfg.Len);
      end loop;
   end Build_Session;
   ...
   begin
      ...
      for I in 1 .. Count loop
         R.Score := R.Score + Trial_Score (Trials (I).Length, Answers (I));
      end loop;
      return R;
   end Score_Session;
end Line_Measure;
