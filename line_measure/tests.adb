...
with Line_Measure; use Line_Measure;
procedure Tests is
...
begin
   ...
   Put_Line ("TEST 1");
   declare
      Cells : constant Row := [Filled, Filled, Empty, Filled, others => Empty];
   begin
      Check ("1.1 count", Count_Filled (Cells, 4) = 3);
      Check ("1.2 score", Trial_Score (3, 3) = 1);
      Check ("1.3 miss", Trial_Score (3, 2) = 0);
   end;
   Put_Line ("TEST 2 — Config");
   Check ("2.1", Config_Ok ((8, 10, 5)));
   Check ("2.2", not Config_Ok ((Max_Trials + 1, 10, 1)));
   Check ("2.3", Instruction_Key'Length > 5);
   ...
   begin
      Build_Session ((8, 10, 5), T, C);
      for I in 1 .. C loop A (I) := T (I).Length; end loop;
      R := Score_Session ((8, 10, 5), T, C, A);
      Check ("4.1", R.Score = R.Max_Score);
      Check ("4.2", R.Max_Score = C);
      Check ("4.3", R.Trials_Run = C);
   end;
   ...
   begin
      Build_Session ((8, 10, 5), T, C);
      for I in 1 .. C loop A (I) := T (I).Length + 1; end loop;
      R := Score_Session ((8, 10, 5), T, C, A);
      Check ("5.1", R.Score = 0);
      Check ("5.2", R.Max_Score = C);
   ...
   begin
      Build_Session ((8, 10, 5), A, CA);
      Build_Session ((8, 10, 5), B, CB);
      Check ("6.1", CA = CB);
      Check ("6.2", CA = CB);
      Check ("6.3", True);
   end;
   ...
   declare
      Z : Session_Config := (8, 10, 5);
      T : Trial_List; C : Natural;
   begin
      Z.Seed := 0;
      Build_Session (Z, T, C);
      Check ("7.1", C = Z.Trial_Count);
      Check ("7.2", Config_Ok (Z));
      Check ("7.3", True);
   end;
   ...
   Check ("9.3", Instruction_Key (1 .. 12) = "line_measure");
   ...
   begin
      Build_Session ((8, 10, 5), T, C);
      for I in 1 .. C loop
         if I mod 2 = 0 then A (I) := T (I).Length;
         else A (I) := T (I).Length + 1; end if;
      end loop;
      R := Score_Session ((8, 10, 5), T, C, A);
   ...
   begin
      ...
      for I in 1 .. C loop A (I) := T (I).Length; end loop;
      R := Score_Session ((8, 10, 5), T, C, A);
      Check ("13.1", R.Score = C);
      Check ("13.2", R.Score = R.Max_Score);
      Check ("13.3", R.Trials_Run = C);
   end;
