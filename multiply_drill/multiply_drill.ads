pragma Ada_2022;
package Multiply_Drill is
   Max_Trials : constant Positive := 32;
   Invalid_Argument : exception;
   type Session_Config is record
      Trial_Count : Positive := 10;
      Max_Factor  : Positive := 12;
      Seed        : Natural := 1;
   end record;
   function Config_Ok (Cfg : Session_Config) return Boolean is
     (Cfg.Trial_Count <= Max_Trials and then Cfg.Max_Factor <= 20);
   type Trial is record A, B, Product : Natural := 0; end record;
   ...
   Instruction_Key : constant String := "multiply_drill.instruction";
   Prompt_Key : constant String := "multiply_drill.prompt_product";
