...
begin
   ...
   declare T : Trial_List; C : Natural; A : Answer_List; R : Session_Result; begin
      Build_Session ((8, 5), T, C); for I in 1 .. C loop A (I) := T (I).Answer; end loop;
      R := Score_Session ((8, 5), T, C, A); Check ("4.1", R.Score = R.Max_Score); Check ("4.2", R.Max_Score = C); Check ("4.3", R.Trials_Run = C); end;
   Put_Line ("TEST 5");
   declare T : Trial_List; C : Natural; A : Answer_List; R : Session_Result; begin
      Build_Session ((8, 5), T, C); for I in 1 .. C loop A (I) := T (I).Answer + 97; end loop;
   ...
   declare A, B : Trial_List; CA, CB : Natural; begin
      ...
      Check ("6.1", CA = CB); Check ("6.2", A (1).Answer = B (1).Answer); Check ("6.3", True); end;
   Put_Line ("TEST 7");
   ...
   Check ("9.3", Instruction_Key (1 .. 12) = "element_pick");
   Put_Line ("TEST 10");
   declare T : Trial_List; C : Natural; A : Answer_List; R : Session_Result; begin
      Build_Session ((8, 5), T, C);
      for I in 1 .. C loop if I mod 2 = 0 then A (I) := T (I).Answer; else A (I) := 0; end if; end loop;
