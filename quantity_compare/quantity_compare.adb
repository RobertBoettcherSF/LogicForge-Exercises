pragma Ada_2022;
package body Quantity_Compare is
   ...
   function Decide (L, R : Natural) return Choice is
   begin
      if L > R then return Left;
      elsif R > L then return Right;
      else return Equal; end if;
   end Decide;
   function Trial_Score (Expected, Given : Choice) return Natural is
   begin
   ...
   begin
      ...
      for I in 1 .. Count loop
         Next_Rand (State, L, 40);
         Next_Rand (State, R, 40);
         Trials (I).Left := L; Trials (I).Right := R;
         Trials (I).Answer := Decide (L, R);
      end loop;
   end Build_Session;
