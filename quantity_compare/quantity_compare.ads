pragma Ada_2022;
package Quantity_Compare is
   Max_Trials : constant Positive := 32;
   type Choice is (Left, Right, Equal);
   ...
   type Answer_List is array (1 .. Max_Trials) of Choice;
   type Session_Result is record
      Config : Session_Config;
      Trials_Run, Score, Max_Score : Natural := 0;
   end record;
   Instruction_Key : constant String := "quantity_compare.instruction";
   Prompt_Key : constant String := "quantity_compare.prompt_which";
   function Decide (L, R : Natural) return Choice with Global => null;
   function Trial_Score (Expected, Given : Choice) return Natural
