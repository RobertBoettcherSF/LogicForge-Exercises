...
with Morse_Decode; use Morse_Decode;
procedure Tests is
...
begin
   ...
   Check ("1.1 hit", Trial_Score (7, 7) = 1);
   Check ("1.2 miss", Trial_Score (7, 8) = 0);
   Check ("1.3 zero", Trial_Score (0, 0) = 1);
   Put_Line ("TEST 2 — Config");
   Check ("2.1", Config_Ok ((8, 5)));
   ...
   begin
      Build_Session ((8, 5), T, C);
      for I in 1 .. C loop A (I) := T (I).Decoded; end loop;
   ...
   begin
      Build_Session ((8, 5), T, C);
      for I in 1 .. C loop A (I) := T (I).Decoded + 97; end loop;
      R := Score_Session ((8, 5), T, C, A);
      Check ("5.1", R.Score = 0);
   ...
   begin
      Build_Session ((8, 5), A, CA);
      Build_Session ((8, 5), B, CB);
      Check ("6.1", CA = CB);
      Check ("6.2", A (1).Decoded = B (1).Decoded);
      Check ("6.3", True);
   end;
   ...
   Check ("9.2", Prompt_Key'Length >= 5);
   Check ("9.3", Instruction_Key (1 .. 12) = "morse_decode");
   ...
   begin
      Build_Session ((8, 5), T, C);
      for I in 1 .. C loop
         if I mod 2 = 0 then A (I) := T (I).Decoded; else A (I) := 0; end if;
      end loop;
      R := Score_Session ((8, 5), T, C, A);
   ...
   Put_Line ("TEST 11");
   Check ("11.1", True); Check ("11.2", True); Check ("11.3", True);
   Put_Line ("TEST 12");
   declare
      T : Trial_List; C : Natural;
   begin
